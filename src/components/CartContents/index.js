...
export function CartContents() {
  ...
  return ( <section>
    <h1> Your cart </h1> {!!checkout?.lineItems && ( 
    <CartHeader >
        <div> Product </div>
         <div> Unit price </div> 
         <div> Quantity </div> 
    ...
    <CartItem key = {item.variant.id} >
        <div>
        <div> {item.title} </div> 
        <div> {item.variant.title === 'Default Title' ? '' : item.variant.title}
         </div> </div> <div > ${item.variant.price} </div>
          <div>
    ...
    } {!checkout?.lineItems && <h4> You cart is empty. </h4>} <Footer>
        <div>
        <Button onClick = {
          () => navigate(-1)
